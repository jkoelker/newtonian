# Modified from khufu_sqlalchemy
# https://github.com/khufuproject/khufu_sqlalchemy
...
DBSESSION = "dbsession"
DBSESSION_ENGINE = "dbengine"
DBSESSION_ENGINE_KWARGS = "dbengine_kwargs"
DBSESSION_FACTORY = "dbsession_factory"
...
def _setup_factory(registry):
    ...
    if DBSESSION_ENGINE in settings:
        ...
        kwargs = {"extension": zope.sqlalchemy.ZopeTransactionExtension(),
                  "bind": engine,
                  "expire_on_commit": False}
        factory = sqlalchemy.orm.sessionmaker(**kwargs)
    ...
    settings[DBSESSION_ENGINE] = engine
...
class _DBSessionFinder(object):
    ...
    def setup_session(self, request):
        ...
        if DBSESSION_FACTORY not in request.registry.settings:
            raise ValueError("Please configure sqlalchemy")
        ...
        if request.registry.settings.get("DEBUG", False):
            ...
            log.debug("created database session for thread: %s (%i active)"
                         % (thread.name, threading.active_count()))

        self.count += 1
        log.debug("db session acquired (%i)" % self.count)

        def close(request, dbsession=environ[DBSESSION]):
            try:
                self.count -= 1
                log.debug("db session closed (%i)" % self.count)
                dbsession.close()
                if hasattr(dbsession, "remove"):
                    log.debug("db session removed (%i)" % self.count)
                    dbsession.remove()
            except Exception, ex:
                log.warning(str(ex))
    ...
    def __call__(self, request, create=True):
        ...
        if (session is None or not _is_active(session)) and create:
            if session is not None and not _is_active(session):
                log.warning("Dropped db session due to being inactive")
        ...
        if session is not None:
            ...
            for x in range(30):
                ...
                if isinstance(obj, sqlalchemy.ext.declarative.DeclarativeMeta):
                    log.debug("merged in the context object to active session")
                    session.merge(obj)
